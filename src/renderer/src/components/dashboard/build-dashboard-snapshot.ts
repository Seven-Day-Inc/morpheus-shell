  ...
  for (const workspace of activeWorktrees) {
    ...
    for (const row of rows) {
      ...
      cards.push({
        ...
        model: nonEmpty(row.entry.model),
        bucket,
        dotState,
        ...
        toolName: isTitleDerived ? undefined : nonEmpty(row.entry.toolName),
        toolInput: isTitleDerived ? undefined : nonEmpty(row.entry.toolInput),
